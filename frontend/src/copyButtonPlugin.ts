...
export default function copyButtonPlugin(md: MarkdownIt) {
    ...
    md.renderer.rules.fence = (tokens, idx, options) => {
      ...
      <div class="code-container group relative">
        <button onclick="window.copyCodeButton(event)" class="copy-btn absolute top-2 right-2 opacity-0 group-hover:opacity-70 transition-opacity p-1 rounded bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500 text-xs z-10 flex flex-row gap-2">
